...
## 🏆 **Betyg: G**
📌 **Motivering:** Projektet 'The Zoo' uppfyller de grundläggande kraven för betyget G. Koden är välstrukturerad och hanterar de flesta funktionaliteter som krävs, såsom routing, hantering av trasiga bildlänkar och grundläggande djurinteraktioner. Dock saknas vissa avancerade tekniker och funktioner som krävs för att uppnå VG, såsom användning av context, reducers och custom hooks för datahantering.

💡 **Förbättringsförslag:**  
För att förbättra projektet och sikta mot VG bör studenten implementera datahämtning med hjälp av tjänster eller en custom hook. Användning av context istället för props för komponentkommunikation och reducers istället för state för djurhantering skulle också förbättra kodens struktur och underhållbarhet. Dessutom bör matningsstatusen för djuren nollställas korrekt vid återgång till djur-sidan, och subtila animeringar kan läggas till för att förbättra användarupplevelsen. Kodkvaliteten kan förbättras genom att minska upprepningar och säkerställa att alla komponenter är välkommenterade och följer naming conventions.